 ...
 * 
 */
document.write('<script src="/js/priceUtils.js"></script>');
...
function priceOpp($listitem) {
	 ...
	 * amount - 만들 옵션의 수량
	 */
	function createTag(optNo,amount){			
		let optName = $("#opt").find("[value="+optNo+"]").data("optname");
		let optPrice = $("#opt").find("[value="+optNo+"]").data("optprice");
				...
				<li class="list-group-item ${optNo} optListItem">
					...
					<span class="float-right mt-2"><span class="optPrice font-weight-bold" data-basicPrice="${optPrice}">${price}</span>원</span>
						<div class="input-group mb-3 optAmount" style="width: 150px;">
							...
							<input type="text" name="amount" class="form-control amount" value="${amount}">
